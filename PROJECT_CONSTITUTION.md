...
`PASS` chỉ dùng khi dữ liệu đã được kiểm và phù hợp.

`ERROR` dùng khi có mâu thuẫn hoặc lỗi xác định được bằng rule hoặc evidence đủ mạnh.

`WARNING` dùng khi có dấu hiệu bất thường nhưng chưa đủ cơ sở kết luận sai.

`MISSING` dùng khi thiếu dữ liệu cần thiết.

`NOT_CHECKED` dùng khi hệ thống chưa thực hiện hoặc không thể thực hiện phép kiểm.

`NEEDS_REVIEW` dùng khi cần con người kiểm tra.

`INSUFFICIENT_EVIDENCE` dùng khi không đủ bằng chứng để kết luận.

Không được biến `UNKNOWN` thành `PASS`.
...
`EXTRACTED`: dữ liệu thực sự xuất hiện trong chứng từ.

`INFERRED`: dữ liệu hệ thống suy ra từ context.
...
Không được gộp tất cả thành `HS CODE`.
...
Không được triển khai toàn bộ nghiệp vụ bằng một chuỗi if/else lớn.
...
```text
Rule A áp dụng từ 2026-01-01
Rule B thay thế từ 2026-07-01
```
...
Nếu file đã được extract trước đó thì reuse extraction result thay vì gọi LLM lại.
...
Dự án phải dần xây một bộ Golden Test Dataset.
...
Trong nghiệp vụ kiểm tra chứng từ, False Negative thường nguy hiểm hơn False Positive.

False Negative:

Có lỗi nhưng hệ thống báo PASS.

Do đó, khi evidence chưa đủ mạnh để PASS, ưu tiên:
...
2. Có giải pháp đơn giản hơn không?
3. Chi phí vận hành?
4. Vendor lock-in?
5. Khả năng bảo trì?

Nếu không có lý do rõ ràng thì không thêm dependency.
...
4. Tests pass.
5. Không có Critical issue chưa xử lý.
6. Reviewer approve hoặc Project Leader override có lý do.
7. Documentation được cập nhật nếu cần.
8. Không chứa secrets.
9. Không phá regression tests.
...
1. Upload một bộ chứng từ.
2. Hệ thống nhận diện từng file.
3. Extract dữ liệu chính.
4. Normalize dữ liệu.
5. So sánh dữ liệu giữa chứng từ.
6. Hiển thị lỗi/cảnh báo.
7. Cho biết nguồn của lỗi.
8. Cho phép người dùng kiểm tra lại.
9. Không cần cấu hình riêng cho từng layout phổ biến.
10. Chi phí xử lý hợp lý.
...
1. Có lý do cụ thể.
2. Không làm giảm độ an toàn của hệ thống mà không có đánh giá.
3. Được Project Owner phê duyệt.
4. Tăng version.
5. Ghi lại thay đổi.
